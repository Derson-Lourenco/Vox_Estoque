...
const PropostaReadequada = ({ userId, contratoId }) => {
  ...
  return (
    <>
      <CCard className="CardTextPrincipal">
        <h2 className="TextPrincipalReadequada">Espelho do Contrato {contratoId}</h2>
      </CCard>
        ...
        <table className="tabelaCentralizada">
          <thead>
            <tr>
              ...
              <th style={{ width: '380px' }}className="TextPrincipalReadequadaTabela">Descrição</th>
              ...
              <th style={{ width: '50px' }} className="TextPrincipalReadequadaTabela">Quant. de Retirar</th>
              <th style={{ width: '80px' }} className="TextPrincipalReadequadaTabela">Ação</th>
            </tr>
          </thead>
          <tbody>
            {propostas.length > 0 ? (
              propostas.map((proposta) => (
                <tr key={proposta.id}>
                  ...
                  <td className="TextPrincipalReadequadaTabelaText">
                  <input
                    className="CardPrincipalAnexoReadequada"
                    type="text"
                    value={quantidadesRetirar[proposta.id] || ''}
                    onChange={(e) => {
                      // Permitir apenas números
                      const value = e.target.value;
                      if (/^\d*$/.test(value)) {
                        handleQuantidadeChange(proposta.id, value ? Number(value) : '');
                      }
                    }}
                    onInput={(e) => {
                  ...
                  <td className="TextPrincipalReadequadaTabelaText">
                    <button
                      ...
                      onClick={() =>
                        handleRetirar(proposta.id, quantidadesRetirar[proposta.id])
                      }btn-limpar
                    >
                      Retirar
                    </button>
...
export default PropostaReadequada;
